...
for i in range(J):
    for j in range(i+1, J):
        ...
        for m in common_m:
            ...
            # j before i
            diff2 = setup_val if pcard[i] != pcard[j] else 0
            model.Add(start[i] >= end[j] + diff2).OnlyEnforceIf(
...
# ------------- 목적 함수 ------------------------------------
avgC   = model.NewIntVar(0, H, 'avgC')
model.Add(avgC == sum(end) // J)
...
if status in (cp_model.OPTIMAL, cp_model.FEASIBLE):
    ...
    print('Avg Completion Time:', solver.Value(avgC)/SCALE)
else:
